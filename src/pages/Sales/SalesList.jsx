...
const SalesList = () => {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      if (data?.success === true || data?.status === true) {
        const message = data.message || 'Pengguna sales berhasil dibuat';
      ...
      let errorText = 'Gagal menyimpan pengguna sales';
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (data?.success === true || data?.status === true) {
        const message = data.message || 'Pengguna sales berhasil dihapus';
        showToast(message, 'success');
        loadPeople();
      } else {
        let errorText = 'Gagal menghapus pengguna sales';
        if (data?.errors && Array.isArray(data.errors) && data.errors.length > 0) {
          errorText = data.errors.join(', ');
        } else if (data?.message) {
          errorText = data.message;
      ...
      {role === 'SUPERVISOR' && (
        <>
          ...
          <ConfirmDelete
            ...
            title="Hapus Pengguna Sales"
            message="Apakah Anda yakin ingin menghapus pengguna sales ini?"
          />
        </>
      )}
